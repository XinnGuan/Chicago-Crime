...
```{r}
Binary_pred_df <- Crime_data %>% select(c("ID","District","Hour","WeekDay","DayOfYear","Primary.Type","Arrest")) %>% filter(District != "31")
#???location variables
```
...
```{r}
base_m <- train(Arrest ~ District + DayOfYear + Hour + 
    Primary.Type, data = imbalanced_train, method = "glm", family = "binomial", trControl = ctrl, metric = "ROC")
```

```{r}
pred <- predict(base_m, imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], type = "prob")
```
...
```{r}
base_m_under <- train(Arrest ~., data = train_balanced_under, method = "glm", family = binomial, trControl = ctrl, metric = "ROC")
base_m_over <- train(Arrest ~ District + DayOfYear + Hour + Primary.Type, data = train_balanced_over, method = "glm", family = "binomial", trControl = ctrl, metric = "ROC")
...
```{r}
pred_under <- predict(base_m_under, imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], type = "prob")
pred_over <- predict(base_m_over, imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], type = "prob")
pred_rose <- predict(base_m_rose, imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], type = "prob")
```
...
```{r}
f2 <- f_beta_score(X=imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], y=imbalanced_test$Arrest,model=base_m,beta=5)
f2_under <- f_beta_score(X=imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], y=imbalanced_test$Arrest,model=base_m_under,beta=5)
f2_over <- f_beta_score(X=imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], y=imbalanced_test$Arrest,model=base_m_over,beta=5)
f2_rose <- f_beta_score(X=imbalanced_test[,c("District", "DayOfYear", "Hour", "Primary.Type")], y=imbalanced_test$Arrest,model=base_m_rose,beta=5)
