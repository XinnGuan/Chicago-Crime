...
(colour="grey80"))+theme(panel.grid.minor=element_line(colour="grey80")) + theme(text = element_text(size=20))
...
for (k_index in seq_along(k_values)) {
  for (run in 1:100) {
    ...
    predicted_crimes <- class::knn(train = train_coordinates, test = test_coordinates, cl = train_data$crimes_within_5km, k = k_values[k_index])
