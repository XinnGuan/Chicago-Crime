...
```{r}
pred <- predict(base_m, imbalanced_test[,c("X.Coordinate", "Y.Coordinate", "DayOfYear", "Hour", "Primary.Type")], type = "prob")
```
...
```
```{r}
binary_predictions <- ifelse(pred_under[,2] >= 0.5, "true", "false")
# create confusion matrix 
confusionMatrix(as.factor(binary_predictions), imbalanced_test$Arrest,
                mode = "everything",
                positive="true")
```
```{r}
binary_predictions <- ifelse(pred_over[,2] >= 0.5, "true", "false")
# create confusion matrix 
confusionMatrix(as.factor(binary_predictions), imbalanced_test$Arrest,
                mode = "everything",
                positive="true")
```
```{r}
binary_predictions <- ifelse(pred_rose[,2] >= 0.5, "true", "false")
# create confusion matrix 
confusionMatrix(as.factor(binary_predictions), imbalanced_test$Arrest,
                mode = "everything",
                positive="true")
...
roc.curve(imbalanced_test$Arrest, pred[,2])
roc.curve(imbalanced_test$Arrest, pred_under[,2])
roc.curve(imbalanced_test$Arrest, pred_over[,2])
roc.curve(imbalanced_test$Arrest, pred_rose[,2])
