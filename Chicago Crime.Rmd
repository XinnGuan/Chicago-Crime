...
for (i in 1:length(k_values)) {
  # Use KNN for regression to predict the number of crimes on the test set
  predicted_crimes <- knn(train = train_coordinates, test = test_coordinates, cl = train_data$crimes_within_5km, k = k_values[i])
  
  # Evaluate the performance (you can use various regression metrics)
  mse_values[i] <- mean((as.numeric(predicted_crimes) - test_data$crimes_within_5km)^2)
}
